...
The detailed per-class mAP values are shown in the confusion matrix and curves generated by Ultralytics:

- results/results.png – training vs validation curves  
- results/confusion_matrix.png – emotion classification performance across 8 classes  

### Example Validation Predictions

Below are sample predictions from the validation set (saved by Ultralytics as val_batch*_pred.jpg), showing detected faces with their predicted emotion labels:

| Example | Image |
|--------|-------|
| Batch 0 predictions | ![](results/val_batch0_pred.jpg) |
| Batch 1 predictions | ![](results/val_batch1_pred.jpg) |
| Batch 2 predictions | ![](results/val_batch2_pred.jpg) |

These examples confirm that the model is not only learning from the data but also producing reasonable emotion predictions on unseen validation images.
